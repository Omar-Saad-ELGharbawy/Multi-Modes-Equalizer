...
with slider_1 :
    ...
    for i in range (50,250):
        st.session_state.yf_array[int(i*points_per_freq)]=st.session_state.slider1*-1j
    # new_sig = irfft(st.session_state.yf_array)
    # norm_new_sig = np.int16(new_sig * (32767 / new_sig.max()))
    # write("piano.wav", sample_rate, norm_new_sig)
    # st.audio("piano.wav",format='audio/wav')
...
with slider_2 :
    ...
    for i in range (270,450):
        st.session_state.yf_array[int(i*points_per_freq)]=st.session_state.slider2*-1j
    # st.write(st.session_state.yf_array[int(i*points_per_freq)]
    st.write("xylophone")
...
with fourier_signal_graph:
    ...
    fig.add_trace(go.Scatter(x=st.session_state.xf_array,
                                ...
                                mode='lines',
                                name='fourier'))
    fig.update_xaxes(showgrid=True, zerolinecolor='black', gridcolor='lightblue', range = (-0.1,5000))
    fig.update_yaxes(automargin=True)
    st.plotly_chart(fig,use_container_width=True)
